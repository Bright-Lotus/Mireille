...
import { EmbedBuilder, bold, underscore, formatEmoji, chatInputApplicationCommandMention, time, TimestampStyles, MessageComponentInteraction, AnySelectMenuInteraction } from 'discord.js';
...
import { Equipment, Equipped, OnAttack } from '../types/equipment.js';
import { ArmorPlate, Bow } from '../types/item.js';

async function abilityModalTriggered(interaction: AnySelectMenuInteraction) {
    // Target handler is for proccessing the target of the ability
    await targetHandler(interaction);
    return;
}

async function allyTargetTriggered(interaction: AnySelectMenuInteraction) {
    const idSplit = interaction.values[ 0 ].split('-');
    const target = idSplit[ 3 ];
    const abilityID = idSplit[ 4 ];

    await ability(Number(abilityID), target, interaction.user).then(results => {
...
async function inventoryBowsModalTriggered(interaction: AnySelectMenuInteraction) {
    ...
    const equipment = await getDoc(doc(db, interaction.user.id, 'PlayerInfo/Inventory/Equipment').withConverter(converter<Equipment>()));
    const equipmentData = equipment.data() as Equipment;
    if (equipment.exists()) {
        // Get the bow from the equipment
        // const bow = equipment.data().bows[ `bow${bowID}` ];
        const bow = equipmentData.bows?.[`bow${bowID}`];
        if (!bow) return;
        const bowStats = bow.stats;
        // Get the currently equipped bow
        // Should return a number, that number is the ID from the equipment
        const equipped = await getDoc(doc(db, interaction.user.id, 'PlayerInfo/Inventory/Equipped').withConverter(converter<Equipped>()));
        const equippedData = equipped.data() as Equipped;

        // This is for removing the stats of the currently equipped bow
        let oldBow: Bow = bow;
        if (!equipped.exists()) return;
        if (equippedData.bow?.id) {
            if (equippedData.bow.id === bowID) {
                return interaction.reply({ embeds: [ErrorEmbed(EventErrors.ItemAlreadyEquipped)], ephemeral: true });
            }
            oldBow = equipmentData.bows?.[`bow${equippedData.bow.id}`] ?? bow;
            // TODO: Delete this comment if the snippet above works
            // await updateDoc(
            //     doc(db, interaction.user.id, 'PlayerInfo'), 'stats.atk', increment(-Math.abs(bowStats.atk)),
            //     [{ 'stats.speed': increment(-Math.abs(bowStats.spd)) }],
            // );
            // await updateDoc(
            //     doc(db, interaction.user.id, 'PlayerInfo'), 'stats.speed', increment(-Math.abs(bowStats.spd)),
            // );
        }
        const finalStats = {
            atk: bowStats.atk - oldBow.stats.atk,
            spd: bowStats.spd - oldBow.stats.spd,
        };
        // Sets the bow ID to the equipped bow
        await updateDoc(
            doc(db, interaction.user.id, 'PlayerInfo/Inventory/Equipped'), 'bow', { id: bowID },
        );

        // Adds the stats of the bow to the player stats
        await updateDoc(
            doc(db, interaction.user.id, 'PlayerInfo'),
            'stats.atk', increment(finalStats.atk),
            ['stats.speed', increment(finalStats.spd)],
        );

        // If the bow has perks, add them to the onAttack array
        if (bow?.perks) {
            const itemPerks: OnAttack[] = [];
            Object.values(bow.perks).forEach(async perk => {
                itemPerks.push({ perkRatio: perk.perkRatio, perk: perk.perk });
            });
            await updateDoc(doc(db, interaction.user.id, 'PlayerInfo/Inventory/Equipped'), 'onAttack', itemPerks);
        }
        ...
        const equippedEmbed = new EmbedBuilder()
            ...
            .setColor('Green')
            .setDescription(
                `Desequipado **${oldBow.name}**
                Equipado **${bow.name}**\n
                **Estadisticas obtenidas**
                **+${finalStats.atk}** ATK ${(Math.sign(finalStats.atk) === 1) ? Icons.StatUp : Icons.StatDown}
                **+${finalStats.spd}** SPD ${(Math.sign(finalStats.spd) === 1) ? Icons.StatUp : Icons.StatDown}`
            );
...
async function inventoryArmorPlatesModalTriggered(interaction: AnySelectMenuInteraction) {
    const armorPlateID = interaction.values[ 0 ].split('-')[ 3 ];
    const equipment = await getDoc(doc(db, interaction.user.id, 'PlayerInfo/Inventory/Equipment').withConverter(converter<Equipment>()));
    const equipmentData = equipment.data() as Equipment;

    if (equipment.exists()) {
        const armorPlate = equipmentData.armorPlates?.[`armorPlate${armorPlateID}`];
        if (!armorPlate) return;
        const armorPlateStats = armorPlate.stats;
        const equipped = await getDoc(doc(db, interaction.user.id, 'PlayerInfo/Inventory/Equipped').withConverter(converter<Equipped>()));
        const equippedData = equipped.data() as Equipped;

        let oldArmorPlate: ArmorPlate = armorPlate;
        if (equipped.exists()) {
            if (equippedData.armorPlate?.id === armorPlateID) {
                return interaction.reply({ embeds: [ErrorEmbed(EventErrors.ItemAlreadyEquipped)], ephemeral: true });
            }
            oldArmorPlate = equipmentData.armorPlates?.[`armorPlate${armorPlateID}`] ?? armorPlate;
        }
        const finalStats = {
            armor: armorPlateStats.armor - oldArmorPlate.stats.armor,
            maxHp: armorPlateStats.maxHp - oldArmorPlate.stats.maxHp,
            magicDurability: armorPlateStats.magicDurability - oldArmorPlate.stats.magicDurability,
        };
        ...
        await updateDoc(doc(db, interaction.user.id, 'PlayerInfo'),
            'stats.armor', increment(finalStats.armor),
            ['stats.magicDurability', increment(finalStats.magicDurability),
            'stats.maxHp', increment(finalStats.maxHp)],
        );
        ...
        if (armorPlate?.perks) {
            ...
            await updateDoc(doc(db, interaction.user.id, 'PlayerInfo/Inventory/Equipped'), 'onEnemyAttack', itemPerks);
        }
        ...
        const equippedEmbed = new EmbedBuilder()
            ...
            .setColor('Green')
            .setDescription(
                `Desequipado **${oldArmorPlate.name}**
                Equipado **${armorPlate.name}**\n
                **Estadisticas obtenidas**
                **+${finalStats.armor}** ARMOR ${(Math.sign(finalStats.armor) === 1) ? Icons.StatUp : Icons.StatDown}
                **+${finalStats.magicDurability}** MAGIC RES ${(Math.sign(finalStats.magicDurability) === 1) ? Icons.StatUp : Icons.StatDown}
                **+${finalStats.maxHp}** MAX HP ${(Math.sign(finalStats.maxHp) === 1) ? Icons.StatUp : Icons.StatDown}`,
            );
        interaction.reply({ embeds: [ equippedEmbed ] });
    }
    return;
...
async function inventoryWandsModalTriggered(interaction: AnySelectMenuInteraction) {
...
async function inventoryAbilityOrbsModalTriggered(interaction: AnySelectMenuInteraction) {
    if (interaction.user.id != interaction.customId.split('/')[ 1 ]) {
        return interaction.reply({ embeds: [ ErrorEmbed(EventErrors.NotOwnerOfInteraction) ], ephemeral: true });
    }
...
async function consumableSelectMenuTriggered(interaction: AnySelectMenuInteraction) {
    if (interaction.user.id != interaction.customId.split('/')[ 1 ]) {
        return interaction.reply({ embeds: [ ErrorEmbed(EventErrors.NotOwnerOfInteraction) ], ephemeral: true });
    }
...
async function inventorySwordsModalTriggered(interaction: AnySelectMenuInteraction) {
    if (interaction.user.id != interaction.customId.split('/')[ 1 ]) {
        return interaction.reply({ embeds: [ ErrorEmbed(EventErrors.NotOwnerOfInteraction) ], ephemeral: true });
    }
...
async function battleFlowSelectMenuTriggered(interaction: AnySelectMenuInteraction) {
    if (interaction.user.id != interaction.customId.split('/')[ 1 ]) {
        return interaction.reply({ embeds: [ ErrorEmbed(EventErrors.NotOwnerOfInteraction) ], ephemeral: true });
    }
    const playerInfo = await getDoc(doc(db, interaction.user.id, 'PlayerInfo'));
...
async function battleFlowActiveBattleTriggered(interaction: AnySelectMenuInteraction) {
...
async function classSelectTriggered(interaction: AnySelectMenuInteraction) {
    if (interaction.user.id != interaction.customId.split('/')[ 1 ]) {
        return interaction.reply({ embeds: [ ErrorEmbed(EventErrors.NotOwnerOfInteraction) ], ephemeral: true });
    }
...
module.exports = {
    ...
    async execute(interaction: MessageComponentInteraction) {
        ...
        interactionHandlers[interactionType as keyof typeof interactionHandlers](interaction);
        ...
        */
        ...
        console.log(`${interactionType} has been triggered!`);
    },
};

async function classSelect(interaction: AnySelectMenuInteraction) {
